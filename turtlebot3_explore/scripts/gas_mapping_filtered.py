...
class gas_mapping:
    ...
    def callback(self, gas_msg, odom_msg):
        position = odom_msg.pose.pose.position
        # visualize the gas map
        pixel_x = int((position.x - self.gas_visual_map.info.origin.position.x) / self.gas_visual_map.info.resolution)
        pixel_y = int((position.y - self.gas_visual_map.info.origin.position.y) / self.gas_visual_map.info.resolution)
        self.gas_visual_map.data[pixel_y * self.gas_visual_map.info.width + pixel_x] = int(gas_msg.gas.data * self.gas_scale + self.gas_offset)
